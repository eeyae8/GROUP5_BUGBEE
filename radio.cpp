                ...
                if (receivedText.find("*") != string::npos) {
                    ...
                    while (emergencyTriggered){
                        if (emergencyCancelRequested) {
                            ...
                            emergencyCancelRequested = false;  // Reset for next time
                            emergencyAudioThread.join();
                            transmit_text_TTT("*Emergency acknowledged.", "transcription.txt");
                        }
                        radioMutex.unlock();
                    ...
                    updateNextion("received_txt", receivedText);
                    playFile("text_notification.wav");
                }
...
void playEmergencyAudio(){
    while (emergencyTriggered){
        ...
        cout << "Emergency ";
        if (emergencyTriggered) cout << "ongoing" << endl;
...
void receiveFile(string receivedFilename) {
    ...
    // use aut because type returned by now is long, use auto to keep it clean
    ...
    // record timestamp after transcription finished
    auto end = chrono::high_resolution_clock::now();
    // calculate difference between end and start time
    // convert to millisecond
    auto duration = chrono:: duration_cast<chrono::milliseconds>(end - start);
    cout << "Data sending time: " << duration.count() << " ms" << endl;
